  ...
  <div class="py-2 px-1 text-gray-300 text-lg">
    <div class="text-base">
      <ul class="breadcrumbs">
        <li><home-icon class="w-5 mb-1 inline hover:text-yellow-200"></home-icon> <a class="hover:text-yellow-200"
             href="/">Home</a></li>
      </ul>
      <ul class="breadcrumbs">
        <li><document-plus-icon class="w-5 mb-1 inline hover:text-yellow-200"></document-plus-icon> <a href="/edit/-1"
             class="hover:text-yellow-200">New Address</a></li>
      </ul>
    </div>
    <div class="join w-full my-1">
      ...
      <button class="btn join-item border-0 bg-neutral"
              @click="store.filter = ''"><x-mark-icon class="w-5"></x-mark-icon></button>
    </div>
      ...
      <ul class="menu text-lg">
        ...
        <li v-for="e in store.entryList" 
          ...
          <div @click="onSelected(e)"
               :class="{ 'text-white': currentId === e.id, 'font-bold': currentId === e.id }">{{ e.displayName
            }}</div>
        </li>
      </ul>
    </div>
